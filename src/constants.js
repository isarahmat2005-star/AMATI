...
- TIMELINE SCENE GENERIK (WAJIB): Jika terdapat lebih dari 1 scene, WAJIB gunakan rumus standar berikut di awal update() untuk membagi waktu (DILARANG menulis if-else manual berpatokan pada angka detik tertentu):
  const sceneCount = [jumlah scene sesuai blueprint];
  const sceneDuration = duration / sceneCount;
  const currentScene = Math.min(sceneCount - 1, Math.floor(t / sceneDuration));
  const sceneProgress = (t % sceneDuration) / sceneDuration;
  Gunakan currentScene dan sceneProgress untuk menentukan animasi tiap elemen per scene. Transisi opacity/transform antar scene WAJIB menggunakan silang (crossfade) agar tidak patah.
...
- KEKAYAAN KOMPOSISI (WAJIB): DILARANG membuat scene yang HANYA berisi 1 objek tunggal yang terisolasi. WAJIB tambahkan minimal 2-3 elemen pendukung/dekoratif (seperti partikel melayang, shadow/bayangan, garis aksen, atau bentuk geometris kecil latar belakang) agar komposisi terlihat kaya, premium, dan padat. Elemen pendukung ini WAJIB tetap minim gerak (statis atau sangat lambat) agar tidak mencuri fokus dari elemen utama.
