...
// =====================================================================
// === HELPER OPFS (Origin Private File System) — PENYIMPANAN LOKAL ===
// =====================================================================
const getOpfsDir = async (kind) => {
    const root = await navigator.storage.getDirectory();
    return root.getDirectoryHandle(kind === 'mp4' ? 'renders_mp4' : 'renders_base64', { create: true });
};

const saveToOpfs = async (kind, cardId, data) => {
...
const getFromOpfs = async (kind, cardId) => {
    const dir = await getOpfsDir(kind);
    const ext = kind === 'mp4' ? 'mp4' : 'txt';
...
const deleteFromOpfs = async (kind, cardId) => {
    try {
        const dir = await getOpfsDir(kind);
        const ext = kind === 'mp4' ? 'mp4' : 'txt';
...
const TrashIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" className={className || ""} width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>;
...
const CopyIcon = ({ className }) => <svg xmlns="http://www.w3.org/2000/svg" className={className || ""} width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>;
...
<body>
    ...
    <script>
        ...
        if (typeof update === 'function' && !isPlaying) update(duration * 0.3, svg, width, height);
...
{ "blueprints": [ { "topic": "Topik Singkat", "visual": "Ringkasan Visual", "scene": "Bagi total durasi (WAJIB sesuai ${builder.duration} detik, JANGAN patokan ke 10 detik) menjadi 3 babak proporsional, sertakan buffer transisi 0.2-0.4 detik di tiap pergantian babak agar mengalir mulus, contoh format: '0-Xs: aksi, X-Ys: aksi, Y-Zs: aksi'", "colors": "Detailkan Proporsi 60-30-10 (Dominan, Sekunder, Aksen)", "background": "Warna dominan 60%..." } ] }`;
    ...
    const handleDownloadZipStreamed = async () => {
        ...
        try {
            ...
            if (err.name !== 'AbortError') setGlobalMessage({ title: "Error Sistem", type: "error", text: "Gagal mengemas file ZIP." });
        } finally {
            setIsZipping(false);
            setZipProgress(null);
        }
                                                                ...
                                                                <div className="flex flex-col gap-2 mb-6 bg-white p-3 border border-slate-200 rounded-lg shadow-sm">
                                                                    {textMatches.map((m, idx) => (
                                                                        <div key={idx} className="flex flex-col gap-1">
                                                                            ...
                                                                            <input type="text" value={m.text} onChange={e => handleSettingsChange('text', e.target.value, m)} className="w-full text-xs py-1.5 px-2 border border-slate-300 rounded bg-slate-50 text-slate-700 focus:ring-2 focus:ring-[#0891B3] focus:bg-white outline-none transition-colors" />
                                                                        </div>
                                                                    ))}
                                                                </div>
                                                            </>
